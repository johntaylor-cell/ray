...
_ENABLE_APPSTATE_CACHE = os.environ.get("RAY_SERVE_ENABLE_APPSTATE_CACHE", "1") != "0"
...
class DeploymentAutoscalingState:
    ...
    def update_running_replica_ids(self, running_replicas: List[ReplicaID]):
        ...
        # Called every control loop tick; membership is usually unchanged, so
        # skip the O(replicas) rebuild and keep the merge cache valid.
        if running_replicas == self._running_replicas:
            return
    ...
    def _calculate_total_requests_aggregate_mode(self) -> float:
        ...
        if (
            not _ENABLE_APPSTATE_CACHE
            or self._merged_ongoing_requests_version != self._metrics_version
        ):
